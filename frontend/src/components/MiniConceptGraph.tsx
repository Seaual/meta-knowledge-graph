  ...
  useEffect(() => {
    ...
    const graph = new ForceGraph(containerRef.current)
      ...
      .nodeCanvasObject((node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
        ...
        ctx.fillStyle = node.type === 'center' ? CENTER_COLOR :
                        node.type === 'parent' ? PARENT_COLOR : CHILD_COLOR
        ctx.fill()
        ...
        if (node.type === 'center' && globalScale > 0.8) {
          ...
          const displayName = node.name.length > 15 ? node.name.slice(0, 15) + '...' : node.name
