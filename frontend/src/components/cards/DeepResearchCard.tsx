  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(135deg, rgba(107, 68, 35, 0.06) 0%, rgba(154, 107, 60, 0.04) 100%)',
        }}
      >
      ...
      {data.report && (
        <div className="p-5 border-b border-academic-border">
          <div className="flex items-center gap-2 mb-3">
            <Lightbulb className="w-4 h-4" style={{ color: 'var(--color-amber)' }} />
            <span className="font-body text-sm font-medium text-academic-sepia">综合结论</span>
          </div>
          <div className="font-body text-sm text-academic-ink leading-relaxed prose-content">
            <ReactMarkdown remarkPlugins={[remarkGfm]}>{removeThinkingTags(data.report)}</ReactMarkdown>
          </div>
        </div>
      )}
      ...
      {dimensions.length > 0 && (
        <div className="p-4 space-y-3">
          <span className="text-xs text-academic-muted font-mono uppercase tracking-wider">各维度分析</span>
              ...
              {expandedDimensions.has(index) && findings[dim] && (
                <div className="px-4 pb-4 pt-2 animate-slide-down">
                  <div className="font-body text-sm text-academic-ink/90 leading-relaxed prose-content">
                    <ReactMarkdown remarkPlugins={[remarkGfm]}>{removeThinkingTags(findings[dim])}</ReactMarkdown>
                  </div>
                </div>
              )}
            </div>
