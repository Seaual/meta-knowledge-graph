...
export const papersApi = {
  list: (status?: string, folder?: string) =>
    api.get("/papers/", { params: { status, folder } }),
  ...
  move: (doi: string, folderId: string) =>
    api.patch(`/papers/${encodeURIComponent(doi)}/folder`, {
      folder_id: folderId,
    }),
  contribution: (doi: string) =>
    api.get(`/papers/${encodeURIComponent(doi)}/contribution`),
  processSingle: (doi: string) =>
    api.post<ProcessSingleResponse>("/papers/process-single", { doi }),
};
