    ...
    <div className="h-full overflow-y-auto">
      <div className="p-8 max-w-5xl mx-auto animate-fade-in">
        ...
        <header className="mb-10">
          <h1
            className="font-display text-2xl mb-1 animate-number"
            style={{ color: "var(--color-ink)" }}
          >
            {t.home.title}
          </h1>
          <p
            className="text-secondary animate-slide-right"
            style={{ fontSize: "0.9375rem", animationDelay: "100ms" }}
          >
            {t.home.subtitle}
          </p>
        </header>
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10 stagger-children">
          {[
            {
              icon: FileText,
              label: t.home.stats.papers,
              value: stats?.papers?.total || 0,
            },
            {
              icon: GitBranch,
              label: t.home.stats.concepts,
              value: stats?.concepts?.total || 0,
            },
            {
              icon: Network,
              label: t.home.stats.relations,
              value: stats?.relations || 0,
            },
            {
              icon: TrendingUp,
              label: t.home.stats.roots,
              value: stats?.root_concepts || 0,
            },
          ].map((stat, i) => (
            <div
              key={i}
        ...
        <section className="grid md:grid-cols-2 gap-4 mb-10">
          <Link to="/papers" className="card-action group block">
          ...
          <Link to="/concepts" className="card-action group block">
            <div className="flex items-center gap-4">
              <div
                ...
                .map(([status, count]) => (
                  ...
                  >
                    ...
                    <p className="font-mono text-xs text-muted capitalize">
                      {t.papers.status[
                        status as keyof typeof t.papers.status
                      ] || status}
                    </p>
                  </div>
                ))}
        ...
        <S2ConfigModal
          ...
          onSave={() => {
            ...
            s2Api.getConfig().then((res) => {
              if (res.data.has_api_key) {
                setS2Status(
                  res.data.enabled ? t.home.enabled : t.home.disabled
                );
              } else {
                setS2Status(t.home.notConfigured);
              }
