...
- **Agent 会话隔离**：聊天请求现在会透传 `conversationId`，LangGraph checkpoint 不再共享同一个全局线程。
- **概念写库提速**：论文概念树会先收集，再在单个数据库事务中批量写入，降低处理过程中的提交开销。
- **外部调用更稳定**：研究点发现、概念翻译和共享文本生成链路都接入了带重试和结构化日志的容错包装。
- **图谱交互性能优化**：概念页会复用同一个 ForceGraph 实例，常见 UI 状态变化不再触发整图销毁重建。
- **补充回归测试**：新增了针对重试行为和 LLM 包装层的基础测试。

详细发布内容见 [CHANGELOG.md](CHANGELOG.md)。
